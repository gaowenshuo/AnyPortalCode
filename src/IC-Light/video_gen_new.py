...
def run_rmbg(img, sigma=0.0):
    ...
    #print(result.clip(0, 255).astype(np.uint8))
    #print(result.clip(0, 255).astype(np.uint8).shape)
    #print(alpha)
    #print(alpha.shape)
...
def main(input_fg_path, input_bg_path, output_path, prompt, image_width, image_height, only_first_frame, using_existing_mask): #
    ...
    n_prompt = "much hair, hairy, lowres, bad anatomy, bad hands, cropped, worst quality"
    cfg = 7.0
    highres_scale = 1.0
    highres_denoise = 0.5
...
if __name__ == "__main__":
    ...
    parser.add_argument("--only_first_frame", action="store_true")
    parser.add_argument("--using_existing_mask", type=str) #

    args = parser.parse_args()
    main(args.input_fg_path, args.input_bg_path, args.output_path, args.prompt, args.width, args.height, args.only_first_frame, args.using_existing_mask) #
