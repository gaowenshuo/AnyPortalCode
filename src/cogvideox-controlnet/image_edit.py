...
parser.add_argument('--image_cond', type=str, required=True, help='Path to the image condition')
...
parser.add_argument('--backward_one_step', action='store_true', help='Run backward one step')
...
image_cond = Image.open(image_cond_path)
...
if args.use_existing_noise:
    ...
    latents = prev_sample
    # for ddim
    if args.backward_one_step:
        alpha_t = alpha_prod_t / alpha_prod_t_prev
        sample = torch.sqrt(alpha_t) * prev_sample + torch.sqrt(1 - alpha_t) * eps
        latents = sample
        warmup = warmup - 1
else:
...
print("Preprocessing image condition")

image_cond = pipe.video_processor.preprocess(image_cond, height=480, width=720).to(
                device, dtype=prompt_embeds.dtype
            )
#print("image_cond:", image_cond)
#print("image_cond:", image_cond.shape) # [1, 3, 480, 720]
image_cond = image_cond.unsqueeze(2)  # [B, C, F, H, W]
with torch.no_grad():
    image_latents = [vae.encode(img.unsqueeze(0)).latent_dist.sample() for img in image_cond]
    image_latents = torch.cat(image_latents, dim=0).to(prompt_embeds.dtype).permute(0, 2, 1, 3, 4)  # [B, F, C, H, W]
    image_latents = vae.config.scaling_factor * image_latents
    a, b, c, d, e = latent_z_refined.shape
    shape = a, b-1, c, d, e
    zero_padding = torch.zeros(shape, device=device, dtype=prompt_embeds.dtype)
    image_latents = torch.cat([image_latents, zero_padding], dim=1)

print("Finished preprocessing image condition")
