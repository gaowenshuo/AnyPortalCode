...
parser.add_argument('--backward_steps', type=int, help='Run backward step')
...
if args.use_existing_noise:
    ...
    if args.backward_steps is not None:
        alpha_prod_t = pipe.scheduler.alphas_cumprod[timesteps[warmup - args.backward_steps]]
        alpha_t = alpha_prod_t / alpha_prod_t_prev
        sample = torch.sqrt(alpha_t) * prev_sample + torch.sqrt(1 - alpha_t) * eps
        latents = sample
        warmup = warmup - args.backward_steps
else:
