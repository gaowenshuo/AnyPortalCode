...
model_path = './src/IC-Light/models/iclight_sd15_fbc.safetensors'

if not os.path.exists(model_path):
    print("Downloading model...")
    download_url_to_file(url='https://hf-mirror.com/lllyasviel/ic-light/resolve/main/iclight_sd15_fbc.safetensors', dst=model_path)
...
def process_relight(input_fg, input_bg, prompt, image_width, image_height, num_samples, seed, steps, a_prompt, n_prompt, cfg, highres_scale, highres_denoise, bg_source):
    ...
    # print(results[0].shape)
    # print(input_bg.shape)
    results = [x * matting + input_bg * (1 - matting) for x in results]
...
def main(input_fg_path, input_bg_path, output_path, prompt):
    input_fg = Image.open(input_fg_path)
    input_bg = Image.open(input_bg_path)
    image_width = 720
    image_height = 480
    #resize input fg and bg
    input_fg = input_fg.resize((image_width, image_height), Image.LANCZOS)
    input_bg = input_bg.resize((image_width, image_height), Image.LANCZOS)
    num_samples = 1
    seed = 12345
    steps = 20
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process images and create a result image.")
    parser.add_argument("--input_fg_path", type=str, required=True, help="Path to the foreground image.")
    parser.add_argument("--input_bg_path", type=str, required=True, help="Path to the background image.")
    parser.add_argument("--output_path", type=str, required=True, help="Path to the output image file.")
    parser.add_argument("--prompt", type=str, required=True, help="Prompt for the image generation.")

    args = parser.parse_args()
    main(args.input_fg_path, args.input_bg_path, args.output_path, args.prompt)
